...
$_SESSION['club']='Sports';
	...
	try
	{
	    ...
	    $all_sports = $pdo->query("SELECT m.MID,m.NAME,m.DEPARTMENT,m.USN,m.PHNO from member as m,club as c where m.CID=c.CID && c.CID=1");
			$all_sports1 = $pdo->query("SELECT e.EID,e.EVENT_NAME,e.PARTICIPATION_COST,e.EVENT_MODE,e.EVENT_DATE,
				e.EVENT_PRIZES from events as e,club as c where e.CID=c.CID && c.CID=1");
			$all_sports2 = $pdo->query("SELECT ex.EXID,e.EVENT_NAME,ex.BUDGET,ex.COLLECTION_COUNT,ex.EXPENSES,ex.PROFIT from expenditure as ex ,
			events as e where e.EXID=ex.EXID and E.CID=1");

		$_SESSION['club']="Sports";
		while ( $row = $all_sports->fetch(PDO::FETCH_OBJ) )
		{
		    $m_sports[] = $row;
		}
		while ( $row1 = $all_sports1->fetch(PDO::FETCH_OBJ) )
		{
				$m_sports1[] = $row1;
		}
		while ( $row2 = $all_sports2->fetch(PDO::FETCH_OBJ) )
		{
				$m_sports2[] = $row2;
		}
	}
...
<html>
  ...
  <body>
  <h3> Welcome TO Sports Database</h3>
	...
	class="close" title="Close Modal">&times;</span>
		...
		<form class="modal-content animate" method="POST">
			...
			<div class="container" style="background-color:#f1f1f1">
				<button type="button" onclick="location.href='sports.php'" class="cancelbtn">Cancel</button>
  ...
  <p>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_sports as $m_sport) : ?>
                      <tr>
												<td><?php echo $m_sport->MID; ?></td>
                        <td><?php echo $m_sport->NAME; ?></td>
            <td><?php echo $m_sport->DEPARTMENT; ?></td>
            <td><?php echo $m_sport->USN; ?></td>
            <td><?php echo $m_sport->PHNO; ?></td>
            <td><a href="./edit/edit_m.php?member_id=<?php echo $m_sport->MID; ?>">Edit</a> / <a href="./delete/delete_m.php?member_id=<?php echo $m_sport->MID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_sports1 as $m_sport1) :?>
            ...
            <td><a href="./edit/edit_e.php?event_id=<?php echo $m_sport1->EID; ?>">Edit</a> / <a href="./delete/delete_e.php?event_id=<?php echo $m_sport1->EID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
	    ...
	    <table>
	      ...
	      <tbody>
	        <?php foreach($m_sports2 as $m_sport2) :?>
	                      <tr>
													<td><?php echo $m_sport2->EVENT_NAME; ?></td>
	                        <td><?php echo $m_sport2->EXID; ?></td>
	            <td><?php echo $m_sport2->BUDGET; ?></td>
	            <td><?php echo $m_sport2->COLLECTION_COUNT; ?></td>
	            <td><?php echo $m_sport2->EXPENSES; ?></td>
							<td><?php echo $m_sport2->PROFIT; ?></td>
	                      </tr>
	                  <?php endforeach; ?>
	      </tbody>
	    </table>
