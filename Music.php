<?php
session_start();
//if (  ! (isset($_SESSION['Cname']) && $_SESSION['Cname']=='Music'))
	//die("ACCESS DENIED");
...
$_SESSION['club']='Music';
	...
	try
	{
	    ...
	    $all_music = $pdo->query("SELECT m.MID,m.NAME,m.DEPARTMENT,m.USN,m.PHNO from member as m,club as c where m.CID=c.CID && c.CID=3");
			$all_music1 = $pdo->query("SELECT e.EID,e.EVENT_NAME,e.PARTICIPATION_COST,e.EVENT_MODE,e.EVENT_DATE,
				e.EVENT_PRIZES from events as e,club as c where e.CID=c.CID && c.CID=3");
			$all_music2 = $pdo->query("SELECT ex.EXID,e.EVENT_NAME,ex.BUDGET,ex.COLLECTION_COUNT,ex.EXPENSES,ex.PROFIT from expenditure as ex ,
				events as e where e.EXID=ex.EXID and E.CID=3");

				$_SESSION['club']="Music";
		while ( $row = $all_music->fetch(PDO::FETCH_OBJ) )
		{
		    $m_music[] = $row;
		}
		while ( $row1 = $all_music1->fetch(PDO::FETCH_OBJ) )
		{
				$m_music1[] = $row1;
		}
...
<html>
	...
	class="close" title="Close Modal">&times;</span>
		...
		<form class="modal-content animate" method="POST">
			...
			<div class="container" style="background-color:#f1f1f1">
				<button type="button" onclick="location.href='music.php'" class="cancelbtn">Cancel</button>
  ...
  <p>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_music as $m_mus) : ?>
                      <tr>
												<td><?php echo $m_mus->MID; ?></td>
                        <td><?php echo $m_mus->NAME; ?></td>
            <td><?php echo $m_mus->DEPARTMENT; ?></td>
            <td><?php echo $m_mus->USN; ?></td>
            <td><?php echo $m_mus->PHNO; ?></td>
            <td><a href="./edit/edit_m.php?member_id=<?php echo $m_mus->MID; ?>">Edit</a> / <a href="./delete/delete_m.php?member_id=<?php echo $m_mus->MID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_music1 as $m_mus1) :?>
            ...
            <td><a href="./edit/edit_e.php?event_id=<?php echo $m_mus1->EID; ?>">Edit</a> / <a href="./delete/delete_e.php?event_id=<?php echo $m_mus1->EID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
		...
		<table>
			...
			<tbody>
				<?php foreach($m_music2 as $m_mus2) :?>
											<tr>
												<td><?php echo $m_mus2->EVENT_NAME; ?></td>
												<td><?php echo $m_mus2->EXID; ?></td>
						<td><?php echo $m_mus2->BUDGET; ?></td>
						<td><?php echo $m_mus2->COLLECTION_COUNT; ?></td>
						<td><?php echo $m_mus2->EXPENSES; ?></td>
						<td><?php echo $m_mus2->PROFIT; ?></td>
