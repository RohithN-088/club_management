...
session_start();
//if (  ! (isset($_SESSION['Cname']) && $_SESSION['Cname']=='Coding'))
	//die("ACCESS DENIED");
...
$_SESSION['club']='Coding';
	...
	try
	{
	    ...
	    $all_code = $pdo->query("SELECT m.MID,m.NAME,m.DEPARTMENT,m.USN,m.PHNO from member as m,club as c where m.CID=c.CID && c.CID=5");
			$all_code1 = $pdo->query("SELECT e.EID,e.EVENT_NAME,e.PARTICIPATION_COST,e.EVENT_MODE,e.EVENT_DATE,
				e.EVENT_PRIZES from events as e,club as c where e.CID=c.CID && c.CID=5");
			$all_code2 = $pdo->query("SELECT ex.EXID,e.EVENT_NAME,ex.BUDGET,ex.COLLECTION_COUNT,ex.EXPENSES,ex.PROFIT from expenditure as ex ,
				events as e where e.EXID=ex.EXID and E.CID=5");

				$_SESSION['club']='Coding';
		while ( $row = $all_code->fetch(PDO::FETCH_OBJ) )
		{
		    $m_code[] = $row;
		}
		while ( $row1 = $all_code1->fetch(PDO::FETCH_OBJ) )
		{
				$m_code1[] = $row1;
		}
		while ( $row2 = $all_code2->fetch(PDO::FETCH_OBJ) )
		{
				$m_code2[] = $row2;
		}
...
<html>
  ...
  <body>
  <h3> Welcome TO Coding Database</h3>
	...
	class="close" title="Close Modal">&times;</span>
		...
		<form class="modal-content animate" method="POST">
			...
			<div class="container" style="background-color:#f1f1f1">
				<button type="button" onclick="location.href='coding.php'" class="cancelbtn">Cancel</button>
  ...
  <p>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_code as $m_cod) : ?>
                      <tr>
												<td><?php echo $m_cod->MID; ?></td>
                        <td><?php echo $m_cod->NAME; ?></td>
            <td><?php echo $m_cod->DEPARTMENT; ?></td>
            <td><?php echo $m_cod->USN; ?></td>
            <td><?php echo $m_cod->PHNO; ?></td>
            <td><a href="./edit/edit_m.php?member_id=<?php echo $m_cod->MID; ?>">Edit</a> / <a href="./delete/delete_m.php?member_id=<?php echo $m_cod->MID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
  ...
  <p>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_code1 as $m_cod1) :?>
                      <tr>
												<td><?php echo $m_cod1->EID; ?></td>
                        <td><?php echo $m_cod1->EVENT_NAME; ?></td>
            <td><?php echo $m_cod1->PARTICIPATION_COST; ?></td>
            <td><?php echo $m_cod1->EVENT_MODE; ?></td>
            <td><?php echo $m_cod1->EVENT_DATE; ?></td>
						<td><?php echo $m_cod1->EVENT_PRIZES; ?></td>
            <td><a href="./edit/edit_e.php?event_id=<?php echo $m_cod1->EID; ?>">Edit</a> / <a href="./delete/delete_e.php?event_id=<?php echo $m_cod1->EID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
	...
	<p>
		...
		<table>
			...
			<tbody>
				<?php foreach($m_code2 as $m_cod2) :?>
											<tr>
												<td><?php echo $m_cod2->EVENT_NAME; ?></td>
												<td><?php echo $m_cod2->EXID; ?></td>
						<td><?php echo $m_cod2->BUDGET; ?></td>
						<td><?php echo $m_cod2->COLLECTION_COUNT; ?></td>
						<td><?php echo $m_cod2->EXPENSES; ?></td>
						<td><?php echo $m_cod2->PROFIT; ?></td>
