...
session_start();
//if (  ! (isset($_SESSION['Cname']) && $_SESSION['Cname']=='Art'))
	//die("ACCESS DENIED");
...
$_SESSION['club']='Art';
	...
	try
	{
	    ...
	    $all_arts = $pdo->query("SELECT m.MID,m.NAME,m.DEPARTMENT,m.USN,m.PHNO from member as m,club as c where m.CID=c.CID && c.CID=2");
			$all_arts1 = $pdo->query("SELECT e.EID,e.EVENT_NAME,e.PARTICIPATION_COST,e.EVENT_MODE,e.EVENT_DATE,
				e.EVENT_PRIZES from events as e,club as c where e.CID=c.CID && c.CID=2");
				$all_arts2 = $pdo->query("SELECT ex.EXID,e.EVENT_NAME,ex.BUDGET,ex.COLLECTION_COUNT,ex.EXPENSES,ex.PROFIT from expenditure as ex ,
				events as e where e.EXID=ex.EXID and E.CID=2");

    $_SESSION['club']="Art";
		while ( $row = $all_arts->fetch(PDO::FETCH_OBJ) )
		{
		    $m_arts[] = $row;
		}
		while ( $row1 = $all_arts1->fetch(PDO::FETCH_OBJ) )
		{
				$m_arts1[] = $row1;
		}
		while ( $row2 = $all_arts2->fetch(PDO::FETCH_OBJ) )
		{
				$m_arts2[] = $row2;
		}
...
<html>
	...
	class="close" title="Close Modal">&times;</span>
		...
		<form class="modal-content animate" method="POST">
			...
			<div class="container" style="background-color:#f1f1f1">
				<button type="button" onclick="location.href='art.php'" class="cancelbtn">Cancel</button>
  ...
  <p>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_arts as $m_art) : ?>
                      <tr>
												<td><?php echo $m_art->MID; ?></td>
                        <td><?php echo $m_art->NAME; ?></td>
            <td><?php echo $m_art->DEPARTMENT; ?></td>
            <td><?php echo $m_art->USN; ?></td>
            <td><?php echo $m_art->PHNO; ?></td>
            <td><a href="./edit/edit_m.php?member_id=<?php echo $m_art->MID ?>">Edit</a> / <a href="./delete/delete_m.php?member_id=<?php echo $m_art->MID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
    ...
    <table>
      ...
      <tbody>
        <?php foreach($m_arts1 as $m_art1) :?>
            ...
            <td><a href="./edit/edit_e.php?event_id=<?php echo $m_art1->EID ?>">Edit</a> / <a href="./delete/delete_e.php?event_id=<?php echo $m_art1->EID; ?>">Delete</a>
            </td>
                      </tr>
                  <?php endforeach; ?>
      </tbody>
		...
		<table>
			...
			<tbody>
				<?php foreach($m_arts2 as $m_art2) :?>
											<tr>
												<td><?php echo $m_art2->EVENT_NAME; ?></td>
												<td><?php echo $m_art2->EXID; ?></td>
						<td><?php echo $m_art2->BUDGET; ?></td>
						<td><?php echo $m_art2->COLLECTION_COUNT; ?></td>
						<td><?php echo $m_art2->EXPENSES; ?></td>
						<td><?php echo $m_art2->PROFIT; ?></td>
